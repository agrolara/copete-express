    ...
    Object.entries(selectedItems).forEach(([id, item]) => {
      if (item.selected) {
        if (item.type === 'product') {
          ...
          if (p) sub += p.price * item.quantity;
        } else {
          const promo = promotions.find((pr) => pr.id === id);
          if (promo) sub += promo.promo_price * item.quantity;
        }
      }
    });
    return sub;
  }, [selectedItems, products, promotions]);

  const liveDiscountAmount = useMemo(() => {
    if (discountType === 'percentage' && discountValue > 0) {
      return Math.round(selectedOrderSubtotal * (Math.min(100, Math.max(0, discountValue)) / 100));
    }
    if (discountType === 'fixed' && discountValue > 0) {
      return Math.min(selectedOrderSubtotal, Math.max(0, discountValue));
    }
    return 0;
  }, [selectedOrderSubtotal, discountType, discountValue]);

  const liveFinalTotal = Math.max(0, selectedOrderSubtotal - liveDiscountAmount);
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-3xl max-w-2xl w-full p-6 shadow-2xl space-y-5 my-8">
            ...
            {generatedSummary ? (
              ...
              <form onSubmit={handleProcessAdminOrder} className="space-y-4">
                ...
                {/* SECCIÓN DE DESCUENTO O PROMOCIÓN ESPECIAL */}
                <div className="p-4 rounded-2xl bg-zinc-950 border border-zinc-800 space-y-3">
                  <div className="flex items-center justify-between">
                    <label className="text-xs font-bold text-white flex items-center gap-1.5">
                      <Tag className="w-4 h-4 text-amber-400" />
                      <span>Descuento Especial al Total (Opcional)</span>
                    </label>
                    <span className="text-[10px] text-zinc-400">Aplica sobre el monto final</span>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    <button
                      ...
                      className={`flex items-center justify-center gap-1 py-2 px-3 rounded-xl text-xs font-bold transition-all border ${
                        discountType === 'percentage'
                          ? 'bg-amber-600/30 border-amber-500 text-amber-300 shadow-neon-orange'
                          : 'bg-zinc-900 border-zinc-800 text-zinc-400 hover:text-white'
                      }`}
                    >
                    ...
                    <button
                      ...
                      className={`flex items-center justify-center gap-1 py-2 px-3 rounded-xl text-xs font-bold transition-all border ${
                        discountType === 'fixed'
                          ? 'bg-emerald-600/30 border-emerald-500 text-emerald-300 shadow-neon-purple'
                          : 'bg-zinc-900 border-zinc-800 text-zinc-400 hover:text-white'
                      }`}
                    >
                  ...
                  <div className="mt-2 pt-2 border-t border-zinc-800/80 flex flex-col gap-1 text-xs">
                    <div className="flex justify-between text-zinc-400">
                      <span>Subtotal ({Object.values(selectedItems).filter(v => v.selected).length} ítems):</span>
                      <span className="font-mono font-bold text-white">${selectedOrderSubtotal.toLocaleString('es-CL')}</span>
                    </div>
                    {liveDiscountAmount > 0 && (
                      <div className="flex justify-between text-amber-400 font-semibold">
                        <span>Descuento aplicado {discountType === 'percentage' ? `(${discountValue}%)` : ''}:</span>
                        <span className="font-mono font-bold">-${liveDiscountAmount.toLocaleString('es-CL')}</span>
                      </div>
                    )}
                    <div className="flex justify-between items-center text-sm pt-1 border-t border-zinc-800">
                      <span className="font-black text-white">Total a Cobrar:</span>
                      <span className="font-mono font-black text-emerald-400 text-base">
                        ${liveFinalTotal.toLocaleString('es-CL')}
                      </span>
                    </div>
                  </div>
                </div>
